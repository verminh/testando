...
public object FlagWar {
    ...
    internal var displayBlock : HashMap<UUID,BlockDisplay> = hashMapOf()

    internal var flagTimer : HashMap<UUID,ArmorStand> = hashMapOf()
    ...
    internal fun createAttackBeacon(
        ...
        // Sky BlockDisplay
        val blockDisplay : BlockDisplay = world.spawnEntity(Location(world,x0.toDouble(),y0.toDouble(),z0.toDouble()),EntityType.BLOCK_DISPLAY) as BlockDisplay
        blockDisplay.block = Bukkit.createBlockData(FLAG_COLORS[progressColor])
        val trans = blockDisplay.transformation
        trans.scale.set(5.0)
        blockDisplay.transformation = trans
        blockDisplay.isGlowing = true
        displayBlock[attacker] = blockDisplay


        // Timer
        val timerLoc = flagBase.location.add(0.5,2.0,0.5)
        val armorstand = world.spawnEntity(timerLoc, EntityType.ARMOR_STAND) as ArmorStand
        armorstand.isInvisible = true
        armorstand.isInvulnerable = true
        armorstand.isSmall = true
        armorstand.isCustomNameVisible = true
        armorstand.customName = "${ChatColor.GOLD}Started"
        flagTimer[attacker] = armorstand
                ...
                for ( z in z0..zEnd ) {
                    ...
                    if ( mat == Material.AIR || SKY_BEACON_MATERIALS.contains(mat) ) {
                        if ( (( y == y0 || y == yEnd ) && ( x == x0 || x == xEnd || z == z0 || z == zEnd )) || // end caps, edges glowstone
                            ...
                            if ( createFrame ) {
                                world.block(x, y, z, SKY_BEACON_FRAME_BLOCK)
                            }
                        }
                        else { // color block
    ...
    internal fun updateAttackFlag(
        ...
        val display = displayBlock[attacker]

        display?.block = Bukkit.createBlockData(FLAG_COLORS[progressColor])
    ...
    internal fun cancelAttack(attack: Attack) {
        ...
        displayBlock[attack.attacker]?.remove()
        displayBlock.remove(attack.attacker)

        // remove flagTimer
        flagTimer[attack.attacker]?.remove()
        flagTimer.remove(attack.attacker)
    ...
    internal fun finishAttack(attack: Attack) {
        ...
        val display = displayBlock[attack.attacker]
        display?.remove()
        displayBlock.remove(attack.attacker)
    ...
    internal fun attackTick(attack: Attack) {
        ...
        if ( progress >= attack.attackTime ) {
            ...
            val totalsecond = (attack.attackTime - progress) / 20
            val minutes = totalsecond / 60
            val seconds = totalsecond % 60
            val time = String.format("%02d:%02d", minutes, seconds)
            flagTimer[attack.attacker]?.customName = "${ChatColor.GOLD}${time}"
